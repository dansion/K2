        ...
        initializer:function(cfg){
         ...
         _addHiddenDom:function(){
             var inputDom = _D.create('<input type="hidden" name='+this.get("jsonName")+" >"),
                 containerDom = Y.get("#"+this.get("containerId"));
...
YUI.add('cdselector',function(Y){
    ...
    Y.extend(Cdselector,Y.Cdselector_Base,{
        ...
        _renderDom:function(){
                /*获得container节点*/
            var containerDom=Y.get("#"+this.get("containerId"));
...
},'1.0.0',{requires:['cdselector-base','json-parse','array']});
...
},'1.0.0',{requires:['cdselector','array']});
...
YUI.add('cdselector_optionByStep',function(Y){
    ...
    Y.extend(Cdselector_OptionByStep,Y.Cdselector_Option,{
        ...
        /*联动事件*/
        _selectChange:function(e,lv,childOrigin,value){
            var temp,k=this.get("originDoms").length,container=Y.get("#"+this.get("containerId")),originAry=this.get("originDoms"), optAry=this.get("dataSource").level[lv+1][value];;
            this.set("ifSelect",0);

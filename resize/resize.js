/*
 * @description: a k2 plugin module dependen by YUI3, for size any element of html excepte image element and textareas. For image or textarea needs an additional div wrap for the img tag or textarea tag.
 * @usage:
 ...
 YUI({
  ...
  modules : {
    'k2-resize' : {
      ...
      requires : ['attribute','node','plugin','dd-plugin','dd-proxy','anim','event-mouseenter']
		}
	  }
 ...
 */
"use strict";
YUI.add('k2-resize', function (Y) {



		var EV_START = 'k2-resize:start',
			EV_END = 'k2-resize:end',
			EV_RESIZE = 'k2-resize:resize',
			EV_DRAG = 'k2-resize:drag',
			EV_BEFORE_RESIZE = 'k2-resize:beforeResize',
        ...
        */
        ...
        Y.extend(Resize, Y.Plugin.Base, {
            ...
            _renderHandles: function () {
				...
				//Y.log(_renderHandle(), "_renderHandle");
            ...
            _addDD: function () {
				...
				//Y.log(el.dd.addHandle(), 'the host')
											
				if (this.get("proxy")) {
					//Y.log(Y.Plugin.DDProxy, "DDProxy");
					el.dd.plug(Y.Plugin.DDProxy, { moveOnEnd: true });
					//Y.log(Y.DD.DDM._proxy, "_proxy");
				...
				//Y.log(this._startResize, 'this._startResize');
            ...
            _startResize: function (ev) {
                ...
                //Y.log(ev, '_startResize');
				
				this.fire(EV_BEFORE_RESIZE);

				var el = this.get("host"),
					ch = el.dd.get("activeHandle");
                    
                //Y.log(ch, 'activeHandle');
            ...
            _getPosition: function (node) {
				...
				// hack for parent node is not body
				
				//Y.log(node.ancestor(), 'node ancestor')
				if (node === this.get('host') && !node.ancestor().test('body')) {
					//Y.log('is not body')
					_width = parseInt(node.getStyle("width"), 10);
					_height = parseInt(node.getStyle("height"), 10);
					_top = parseInt(node.getY(), 10) - parseInt(node.ancestor().getY(), 10);
					_left = parseInt(node.getX(), 10) - parseInt(node.ancestor().getX(), 10);
					
				} else {
					//Y.log('is body')
					_width = parseInt(node.getStyle("width"), 10);
					_height = parseInt(node.getStyle("height"), 10);
					_top = parseInt(node.getY(), 10);
					_left = parseInt(node.getX(), 10);
				}
	...
	}, '1.0.0', { requires: ['attribute', 'node', 'plugin', 'dd-plugin', 'dd-constrain', 'dd-proxy', 'anim', 'event-mouseenter'] });

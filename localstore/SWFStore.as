package  
{
	...
	public class SWFStore extends Sprite{
	  ...
	  /**
		...
		*/
		...
		protected function initializeCallbacks(){
			bridge.addCallbacks ({
				...
				calculateCurrentSize:calculateCurrentSize,
				setItem:setItem,
		...
		*/
		...
		private function onComplete (event:Event) : void {
			...
			try{
				...
				bridge.sendEvent({type:"error", message:error.message});
			}
			...
			//not a valid whitelist, dispatch error
				var evt:Object = {type: "securityError",
					message: "Security Error: the whitelist does not allow access to storage from this location" };
		...
		protected function isValidHost():Boolean{
			return true;
		}
		...
		protected function initializeSharedObject():void{
			...
			var loc:String = "DataStore_" + (_shareData?"":browser);
 			...
 			//if(!_initialized)
 			{
 				...
 				bridge.sendEvent({type:"contentReady"});
 			}
		...
		*/
	    protected function save( location:String = null, info:String = "add", oldValue:Object = null, 
						newValue:Object = null, index:int = -1):Boolean{     
	        var evt:Object = {};

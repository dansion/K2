	...
	if(is_uploaded_file($file['tmp_name'])){
		...
		if($type != "image/png" && $type != "image/gif"){
			...
			if(move_uploaded_file($file['tmp_name'],$path.$fileName)){
				...
				$messgae = "<aside class='demo-option-info'><p class='demo-success-info'>Create icon success！<a href='http://kxt.koubei.com/k2/icon/icon.php'>K2 icon demo</a></p></aside>";
				return;
			}else{
...
file.onchange = title.onkeyup = title.onblur = function(){
	disableBtn();
}
</script>
</body>
